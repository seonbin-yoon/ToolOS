...
CONSOLE_CLEAR = "\033[2J\033[3J\033[H"
...
def main(args: list[str]):
    number_to_convert_len = len(args) - 1
    if number_to_convert_len < 1:
        raise ArgNotFoundError("변환할 숫자가 1개 이상 입력되어야합니다.")

    for i, value in enumerate(args[1:]):
        print(CONSOLE_CLEAR, end="")
        try:
            show_converted(i + 1, number_to_convert_len, convert_all(value))
        except ValueError as e:
            print(f"[{i + 1}/{number_to_convert_len}] {str(e)}")

        message = \
            "스페이스를 제외한 아무 키나 눌러 다음으로 넘어갑니다.." \
            if i + 1 < number_to_convert_len \
...
def show_converted(num: int, total: int, info: ConversionInfo):
    match info.num_system:
    ...
    print(f"[{num}/{total}] '{info.value}'는 {num_system}입니다.\n")
